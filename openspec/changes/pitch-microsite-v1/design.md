...
### Experimental interaction direction: staged HTML + restrained 3D atmosphere

**目前實驗方向**：先在 Omma 匯出的單檔 HTML 上，快速驗證「劇場式提案 microsite」是否成立，再決定哪些效果要搬回正式 Next.js repo。

**目前已驗證有效的前端技術組合**：

- `HTML` 作為敘事骨架與舞台裝置容器
- `CSS 3D transforms` 建立假 3D 空間與前後景
- `JavaScript` 控制 scroll-driven composition 與舞台裝置推進
- `Three.js` 負責需要真正立體感的 proposal micro-worlds
- `atmospheric layers`（薄霧、灰塵、光束、地板殘光）增強劇場感，但不蓋過文字

**理由**：這個案子真正吸引人的，不是一般「網站加 3D」，而是「HTML 本身像被放進舞台裡演出」。因此比起滿版粒子或常見科技特效，更適合走 `DOM as scenery`、`scroll as stage transition`、`editorial typography as space` 的方向。

### Creative syntax / frontend techniques worth keeping on the table

以下技術不一定全部都要做，但都值得在本案內保留為可選方向：

- `perspective`, `transform-style: preserve-3d`, `translateZ`, `rotateX`, `rotateY`
- `radial-gradient`, `linear-gradient`, `filter: blur`, `mix-blend-mode`, `backdrop-filter`
- pseudo-elements `::before` / `::after` 建立額外舞台層與燈光層
- `clip-path` / `mask-image` 做舞台切幕與 reveal
- `SVG filters`（如 `feTurbulence`, `feDisplacementMap`）做極輕的布幕或熱氣擾動
- `canvas` overlays 做更自然的 dust / fog / soft light
- `Three.js CSS3DRenderer` 或同類策略，讓 HTML 與 3D 更緊密地放進同一場景
- scroll-synced camera feeling：不一定真的移動 camera，但畫面要有鏡頭推進感

### High-creativity directions that fit this project

以下方向和 Ridge Studio 的品牌氣質相符，且比一般網站特效更有記憶點：

1. `Research wall / archive wall`
   - 適合 Act II / III
   - 讓平台資料、觀察片段、引言像被釘在研究牆上，而不是平面排版

2. `Stage machinery`
   - 適合 Act I / IV / VII
   - 讓 transition 像換景、吊掛、滑軌、轉台，而不是普通 section reveal

3. `Suspended editorial objects`
   - 適合 Act IV / VI
   - 讓文字板、資料板、fragment card 都像實體物件

4. `Atmospheric editorial space`
   - 適合全站
   - 低對比光束、輕薄霧、灰塵、殘光、地板反射

5. `Typography as performance`
   - 適合 Act I / IV
   - 大字本身就是演出，不只是資訊載體

### Practical recommendation

若目標是「讓介面變得生動有趣」但仍保有高級感，實作優先順序建議為：

1. 先強化 `DOM in space`
2. 再補 `atmosphere`
3. 最後才考慮更重的 shader / canvas / experimental DOM-in-3D 技術

理由：這個案子最怕的是「效果很多但很像 AI 樣板網站」。先把構圖、敘事與空間感做對，創意才會成立。
